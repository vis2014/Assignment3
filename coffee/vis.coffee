...
Bubbles = () ->
  ...
  updateLabels = () ->
    ...
    #  styling divs
    label
      .style("font-size", (d) -> Math.max(8, rScale(rValue(d) / 10)) + "px")
      .style("width", (d) -> 2.5 * rScale(rValue(d)/10) + "px")
  ...
  updateActive = (id) ->
    ...
    if id.length > 0
      d3.select("#status").html("<h3>节点 <span class=\"active\">#{id}</span> 是活跃的</h3>")
    else
      d3.select("#status").html("<h3>无节点活跃</h3>")
...
texts = [
  {key:"涨价",file:"increase.csv",name:"涨价商品"}
  {key:"降价最多商店",file:"max_reduce_store.csv",name:"降价最多商店"}
  {key:"降价服装类",file:"clothes_reduce.csv",name:"降价服装类"}
  {key:"降价类化妆品",file:"makeup.csv",name:"降价类化妆品"}
  {key:"降价类数码产品",file:"phone.csv",name:"降价类数码产品"}
]
