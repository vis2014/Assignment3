...
var rotationTab = d3.select("[name=rotation]")
    .on("change", function () {
        rotaterange = d3.select("[name=rotation]").property("value");
        if (d3.select("#text1").property("value") && d3.select("#text2").property("value")) generate();
    });
...
var layout = d3.layout.compcloud()
    ...
    .rotate(function () {
        return (Math.random() * rotaterange * 2 - rotaterange) * 30;
    })
    .fontSize(function (d) {
...
// From Jonathan Feinberg's cue.language, see lib/cue.language/license.txt. Via Jason Davies.
...
/**
 * Eliminating duplicate elements in an array.
...
function draw(words) {
    ...
    d3.select("#viz").append("svg").attr("width", width).attr("height", width * whratio)
        .append("g").attr("transform", "translate(0,0)").selectAll("text").data(words)
